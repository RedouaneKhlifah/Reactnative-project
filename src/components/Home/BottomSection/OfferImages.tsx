...
const ImageComponent: FC<{data: IoffreImagesData}> = ({data}) => {
  return (
    <Image
      style={{width: responsiveWidth(298), height: responsiveHeight(282)}}
      source={data.image}
    />
  );
};
...
const OfferImages: FC<{data: IoffreImagesData[]}> = ({data}) => {
  ...
  const renderItem = ({item}: {item: offreImagesDataT}) => (
  ...
  return (
    <View>
      ...
      />
      <ImagesPagination data={data} scrollX={scrollX} index={index} />
    </View>
  );
};
